...
- [x] **Phase 3 — Image Analyser** ✓ — EfficientNet-B0 fine-tune (`train.py`); data via `prepare_data.py` (kagglehub, 500/class, 7 classes). **Room-type val accuracy 84.4% (>75% ✓) — reproducible via `eval.py` → `eval_metrics.json`** (seed-42 split). Added a **`not_a_room` reject class** for OOD robustness. Service :8002 loads `model.pth`. See [`docs/model_card.md`](property-triage-system/docs/model_card.md). ✓ condition-score head **trained** (second head, Gemini-bootstrapped labels) — the served condition score is provided by **Gemini Vision** (hybrid; the trained head is the spec deliverable + offline fallback).
